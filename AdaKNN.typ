...
#t 下面在汇集区域又增加了一些点，如下是新的数据集：

#figure(
  image("./images/more.png", width:70%),
  caption: "增多点数，新的数据集"
)
...
#align(center)[#tablex(
  ...
  /* --- header --- */
  rowspanx(1)[迭代次数], rowspanx(1)[训练集上的错误率],[测试集上的错误率],
  /* -------------- */
  [3],[0.0526],[0.1],
  [5],[0.1053],[0.1],
  [10],[0.0526],[0.1],
...
更改K近邻的K值为5后，即上述代码中取knn(5)，结果在原始数据集上表现更加糟糕，取knn(7)，比5更加糟糕

= Summary

#t 由于数据集需要DIY，因此数据集还是偏小了，如果数据集更多会更能体现模型的优劣

#t 就目前情况来看，根据这种方法迭代20次左右已经可以达到较好的预测效果，不需要更大的迭代次数几乎是没有意义的

#t 此外，knn中k值的选取也很重要，根据调参结果，取k为3左右会有较好的效果（因为我的样本点数也较少），k大概是总的点数的8%左右较好
